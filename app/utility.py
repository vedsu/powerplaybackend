...
class Utility:
    ...
    def get_fixture():
        match_data = []
        try:
            team_collections  = list(mongo.db.registration.find({},{"_id":0,"RegNo":1, "TeamName":1, "TeamLogo":1}).sort({"TeamName":1}))
    ...
    @staticmethod
    def get_details_by_team(regno):
        player_data = []
        try:
            team_data = list(mongo.db.teams.find({"T_id":regno}))
            team = team_data[0]
            team_dict = {
